...
class Request {
  ...
  async(isasync) {
    this.opts.async = isasync === false ? false : true;
    return this;
  }
  ...
  each(func, isasync) {
    ...
    isasync = isasync || this.opts.async || false;
    let batch = new Batch().concurrency(this.opts.concurrency);
  ...
  map(func, isasync) {
            ...
            this.s3.get(bucket, key, this.encoding, this.transformer).then(val => {
              if (isasync) {
                func(val, source.key).then(newval => {
                  output(bucket, key, source.prefix, newval, done);
                }).catch(done)
              } else {
    ...
    function output(bucket, key, prefix, body, done) {
      if (body == null) {
      ...
      if (self.target == null) {
        ...
        let outputBucket = self.target.bucket;
        let outputKey = key.replace(prefix, self.target.prefix);

        self.s3.put(outputBucket, outputKey, body).then(() => {
          done();
        }).catch(e => {
          done(e);
        })
            ...
            this.s3.get(bucket, key, this.encoding, this.transformer).then(body => {
              if (isasync) {
                func(body, source).then(result => {
                  check(result);
                  if (result) {
                    keep(source).then(_ => done()).catch(done);
                  } else {
                    remove(source).then(_ => done()).catch(done);
                  }
                }).catch(done);
              } else {
                let result = null;
    ...
    function keep(source) {
      return new Promise((success, fail) => {
        if (self.target != null) {
          let bucket = source.bucket;
          let key = source.key;
          let targetBucket = self.target.bucket;
          let targetKey = key.replace(source.prefix, self.target.prefix)
          self.s3.copy(bucket, key, targetBucket, targetKey)
            .then(() => success())
            .catch(fail);
        } else {
          success();
