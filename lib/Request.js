...
class Request {
   ...
   * Make function async
   *
   * @param {Boolean} isAsync Tells s3renity that the batch function is async.
   */

  async(isAsync) {

    // allow .async() to default to true
    this.opts.async = (isAsync !== false)
    return this
  }
  ...
  map(func, isAsync) {
    ...
    // Used to output from the map function (s3renity.context.output.map)
