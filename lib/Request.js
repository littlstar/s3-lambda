...
class Request {
  ...
  /**
   * Rename the key (if rename function exists) when used in map or filter.
   *
  ...
  rename(key) {
    // Rename if rename function exists
    if (this.target.rename) {
      const prefixIndexBeg = key.indexOf(this.target.prefix)
      const prefixIndexEnd = prefixIndexBeg + this.target.prefix.length
      let file = key.slice(prefixIndexEnd)

      file = this.target.rename(file)
      key = key.slice(0, prefixIndexEnd) + file
    }
   ...
   * @param {Function} rename Optional. The function to rename the output key. Defaults to `null`.
   * @return {Request} The instance on which this method was called.
   */
  ...
  map(func, isAsync) {
    ...
    const mapOutput = (bucket, key, prefix, body, done) => {
      ...
      if (this.target == null) {
        ...
        outputKey = this.rename(outputKey)
  ...
  filter(func, isAsync) {
    ...
    const keep = source => new Promise((success, fail) => {
      if (this.target == null) {
        ...
        // Rename output key (if necessary)
        targetKey = this.rename(targetKey)
