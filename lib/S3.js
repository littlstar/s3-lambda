...
class S3 {
  ...
  constructor(config) {
    ...
    if (config.localPath || config.local_path) {
      ...
      if ((config.accessKeyId && config.secretAccessKey) || (config.access_key_id && config.secret_access_key)) {
        aws.config.update({
  ...
  get(bucket, key, transformer) {
    ...
    if (transformer == null) {
      transformer = obj => {
        return obj.Body.toString(this.encoding);
      };
    }
  ...
  keys(bucket, prefix, endPrefix, marker) {
    ...
    return new Promise((success, fail) => {
      listRecursive(bucket, prefix, marker, err => {
        if (err) {
          fail(err);
        } else {
    ...
    function listRecursive(bucket, prefix, marker, done) {
      self.s3Instance.listObjects({
        Bucket: bucket,
        Prefix: prefix,
        Marker: marker
      }, (err, keys) => {
        if (err) {
          ...
          if (keys.Contents.length == 0) {
            ...
            if (keys.IsTruncated && !stopPrefixReached) {
              ...
              listRecursive(bucket, prefix, marker, done);
            } else {
              done();
            }
          }
