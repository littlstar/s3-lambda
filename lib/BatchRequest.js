...
class BatchRequest {
  ...
  constructor(ctx) {
    if (!ctx instanceof Context) {
      throw new TypeError('Must initialize a BatchRequest with a Context');
    }
   ...
   * @param {Boolean} isAsync Whether `func` is async or not.
   * @return {Promise}
   */
  ...
  forEach(func, isAsync) {
    ...
    const _eachObject = (keys, callback) => {
      ...
      S3.get(this.bucket, key).then(body => {
        if (isAsync) {
          func(body).then(_ => {
    ...
    const _splitObjects = (keys, callback) => {
      ...
      const key = keys.shift();
      S3.splitObject(this.bucket, key, this.delimiter, this.encoding)
        .then(entries => {
    ...
    return new Promise((success, fail) => {
      S3.list(this.bucket, this.prefix, this.marker).then(keys => {
  ...
  map(func, isAsync) {
    ...
    const _splitObjects = (keys, callback) => {
      ...
      let key = keys.shift();
      S3.splitObject(this.bucket, key, this.delimiter, this.encoding).then(
        entries => {
    ...
    const _output = (key, body, keys, callback) => {
      if (this.target != null) {
        this
          .put(this.target.bucket, this.target.prefix + S3.getFileName(key), body)
    ...
    const _finish = callback => {
      if (this.hasTarget) {
        ...
        keepObjects.forEach(key => {
          let fileName = S3.getFileName(key);
        ...
        .then(entries => {
          _filterSplitObject(entries).then(newEntries => {
            ...
            if (this.hasTarget) {
              ...
              targetKey = this.targetPrefix + S3.getFileName(key);
            } else {
